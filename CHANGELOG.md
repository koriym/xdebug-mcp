...
### Fixed
- Tools no longer break (or silently analyze stale `/tmp` output) when `XDEBUG_MODE`/`XDEBUG_CONFIG`/`XDEBUG_TRIGGER` is inherited from the environment: spawned child processes now pin their own Xdebug env (`XDEBUG_MODE=<tool mode>`, config/trigger cleared), which takes precedence over both inherited env and `-d` flags, and a one-line notice is emitted when inherited variables are overridden (#94). This also fixes the 3 integration tests that failed under `XDEBUG_MODE=coverage`, so the test-side env scrub in `McpServerIntegrationTest` is removed.
